...
if (process.env.LLAMA_SANITIZE)
  Object.keys(process.env).forEach((key) => {
    ...
    if (
      ["TVL_LOCAL_CACHE_ROOT_FOLDER", "LLAMA_DEBUG_MODE", "GRAPH_API_KEY", "LLAMA_DEBUG_LEVEL2", ...ENV_KEYS].includes(
        key,
      ) ||
      key.includes("SDK")
    )
...
async function getTvl(
  ...
  if (typeof usdTvls[storedKey] !== "number" || Number.isNaN(usdTvls[storedKey])) {
    throw new Error(`TVL for key ${storedKey} is not a number, instead it is ${usdTvls[storedKey]}`);
  }
}
...
(async () => {
  ...
  if (module.hallmarks) {
    ...
    if (module.hallmarks.length > 6) {
      console.error(
        "WARNING: Hallmarks should only be set for events that led to a big change in TVL, please reduce hallmarks to only those that meet this condition",
      );
    }
  ...
  if (
    ...
    (filePath.length === 2 &&
      !(
        ...
        ) // matches .../projects/projectXYZ/v1.js
      ))
  )
    process.exit(0);
